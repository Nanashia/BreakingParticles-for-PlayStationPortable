...
void ExceptionHandler(PspDebugRegBlock * regs)
{
    ...
    pspDebugScreenPrintf("\n\n\n\nPut this in a shell to know where its line is :\n\n"
                         "\tpsp-addr2line -e target.elf -f -C 0x%x 0x%x\n",
                         ( (int)regs->epc - (int)_main ) & 0xFFFFFFF,
                         ( (int)regs->r[31] - (int)_main ) & 0xFFFFFFF);

    pspDebugScreenPrintf("\n\n\n_main(): %p\n",_main);
    pspDebugScreenPrintf("main(): %p",main);
